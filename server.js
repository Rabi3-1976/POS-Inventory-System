...
// ADMIN RESET - CLEAR TRANSACTIONS ONLY
app.post("/admin/clear-transactions-only", verifyToken, adminOnly, async (req, res) => {
    const { confirm_text } = req.body;

    if (confirm_text !== "CLEAR TRANSACTIONS") {
        return res.status(400).json({
    ...
    try {
        ...
        // Clear transaction/detail tables first
        await client.query("DELETE FROM invoice_items");
        await client.query("DELETE FROM invoices");

        // If sales table exists
        await client.query(`
            DO $$
            BEGIN
                IF EXISTS (
        ...
        await client.query(`
            ...
            BEGIN
                IF EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_name = 'profit_transfers'
                ) THEN
                    DELETE FROM profit_transfers;
                END IF;
            END $$;
        `);
        ...
        await client.query(`
            ...
            BEGIN
                IF EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_name = 'receiving_history'
                ) THEN
        ...
        await client.query(`
            ...
            BEGIN
                IF EXISTS (
                    ...
                    UPDATE branch_stock SET stock = 0;
                END IF;
            END $$;
        `);
        ...
        res.json({
            message: "Transactions cleared successfully. Stock reset to zero."
        });
        ...
        res.status(500).json({
            error: "Clear transactions failed: " + err.message
        });
    } finally {
        client.release();
    }
