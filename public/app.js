...
window.clearTransactionsOnly = async function () {
    const confirmText = document.getElementById("clearTransactionsConfirm").value.trim();

    if (confirmText !== "CLEAR TRANSACTIONS") {
        alert("Please type exactly: CLEAR TRANSACTIONS");
        return;
    }

    const firstConfirm = confirm(
        "This will delete all transactions and reset all stock to zero. Continue?"
    );

    if (!firstConfirm) return;

    const secondConfirm = confirm(
        "Final confirmation: this action cannot be undone unless you have a database backup. Continue?"
    );

    if (!secondConfirm) return;

    const res = await fetch(API + "/admin/clear-transactions-only", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "Authorization": "Bearer " + localStorage.getItem("token")
    ...
    document.getElementById("clearTransactionsConfirm").value = "";
    ...
    if (typeof loadCustomerReturns === "function") loadCustomerReturns();
    if (typeof loadPurchaseOrders === "function") loadPurchaseOrders();
    if (typeof loadSupplierReturns === "function") loadSupplierReturns();
    if (typeof loadExpenses === "function") loadExpenses();
    if (typeof loadStockAdjustments === "function") loadStockAdjustments();
    if (typeof loadStockAuditReport === "function") loadStockAuditReport();
};